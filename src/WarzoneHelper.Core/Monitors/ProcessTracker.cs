...
namespace WarzoneHelper.Core.Monitors
{
    ...
    /// COD_PROCESS_STARTED / COD_PROCESS_STOPPED. Other monitors query CurrentPids().
    /// </summary>
    public sealed class ProcessTracker : IMonitor
    {
        ...
        private void Poll()
        {
            try
            {
                ...
                {
                    _wasRunning = running;
                    _bus.Publish(running ? EventNames.CodProcessStarted : EventNames.CodProcessStopped,
