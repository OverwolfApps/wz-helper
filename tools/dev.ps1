...
$task = "GameHelper-Console"
$dotnet = "C:\Program Files\dotnet\dotnet.exe"

try { Stop-ScheduledTask -TaskName $task -ErrorAction Stop | Out-Null } catch {}
Get-Process GameHelper.Console -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
Start-Sleep -Milliseconds 400

& $dotnet build "$root\src\GameHelper.slnx" -c $Configuration 2>&1 | Select-Object -Last 4
